...
TaskClassif = R6Class("TaskClassif",
  ...
  active = list(
    ...
    positive = function(rhs) {
      ...
      if (length(lvls) != 2L) {
        error_config("Setting the positive class is only feasible for binary classification")
      }
...
update_classif_property = function(self, private) {
  ...
  if (fget_key(self$col_info, tn, "type", key = "id") %nin% c("factor", "ordered")) {
    error_config("Target column '%s' must be a factor or ordered factor", tn)
  }

  nlvls = length(self$class_names)
  if (nlvls < 2L) {
    error_config("Target column '%s' must have at least two levels", tn)
  }
