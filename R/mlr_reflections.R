...
local({
  ...
  mlr_reflections$learner_predict_types = list(
    classif = list(response = "response", prob = c("response", "prob"), impact = c("response", "impact")),
    regr = list(response = "response", se = c("response", "se"), distr = c("response", "se", "distr"), impact = c("response", "impact"))
  )
