...
* Result containers `ResampleResult` and `BenchmarkResult` now default to remove
  the DataBackend of the Tasks in order to reduce file size and memory
  footprint after serialization. If you want to keep the backends in the
  containers, set `store_backends` to `TRUE` in `resample()` or `benchmark()`,
  respectively.
