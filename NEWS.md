...
* fix: `resample()` and `benchmark()` with `store_models = FALSE` now also discard the model, the training task copy, and the data prototype of the fallback learner, which were kept for every iteration (#1510).
