...
for objects of type
[Prediction](https://mlr3.mlr-org.com/dev/reference/Prediction.md). It
is an internal data structure, implemented to optimize runtime and solve
some issues emerging while serializing R6 objects. End-users typically
do not need to worry about the details, package developers are advised
to continue reading for some technical information.
...
- [`as_prediction()`](https://mlr3.mlr-org.com/dev/reference/as_prediction.md)
  ...
  [Prediction](https://mlr3.mlr-org.com/dev/reference/Prediction.md),
  e.g. objects of type `PredictionData`.

- `check_prediction_data()` is called on the return value of the predict
  method of a
  [Learner](https://mlr3.mlr-org.com/dev/reference/Learner.md) to
  perform assertions and type conversions. Returns an update object of
  class `PredictionData`.

- `is_missing_prediction_data()` is used for the fallback learner (see
  [Learner](https://mlr3.mlr-org.com/dev/reference/Learner.md)) to
  impute missing predictions. Returns vector with row ids which need
  imputation.
