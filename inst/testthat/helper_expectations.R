...
expect_benchmark_result = function(bmr) {
  ...
  tab = bmr$aggregated
  checkmate::expect_data_table(tab, ncol = 4L + nrow(bmr$measures))
  checkmate::expect_names(names(tab), identical.to = c("hash", "task_id", "learner_id", "resampling_id", bmr$measures$measure_id))
