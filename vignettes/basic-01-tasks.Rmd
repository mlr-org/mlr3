...
They are stored in the dictionary (key-value store) `mlr.tasks`.
...
The row names of `mtcars` are automatically used as row ids.
```{r}
task$row.ids()
...
3. The row id. This column is there for technical reasons but is typically useless for learning.
...
task$data(rows = 1:2, filter.cols = FALSE)
```
