...
For this purpose, `mlr3` ships with the learner `classif.unittest`:
```{r}
...
task = mlr_tasks$get("spam")
learner = mlr_learners$get("classif.unittest")
learner$param_vals = list(warning_train = TRUE, error_train = TRUE)
...
task = mlr_tasks$get("spam")
learner = mlr_learners$get("classif.unittest")
learner$param_vals = list(segfault_train = TRUE)
e = Experiment$new(task, learner)
e$train(ctrl = ctrl)
e$has_errors
e$logs$train$errors
