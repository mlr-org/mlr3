...
To train the learner on the task, we need to call the train function of the experiment and specify :
...
The printer indicates that the `Experiment` object has been modified (its state is now `[trained]`) and has also been extended, since the object now includes a `rpart` model:
