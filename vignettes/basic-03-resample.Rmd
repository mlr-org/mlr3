...
2. The default ratio for splitting into training and test set is $0.67$
...
* Extract and inspect the now instantiated resampling:
    ...
    rr$resampling
    rr$resampling$iters
    rr$resampling$test.set(1)
    rr$resampling$test.set(2)
    rr$resampling$test.set(3)
    ```
...
```{r}
resampling = mlr.resamplings$get("cv", folds = 3)
resampling$instantiate(task)
resampling$iters
resampling$train.set(1)
```
...
```{r}
learner1 = mlr.learners$get("classif.rpart")
learner2 = mlr.learners$get("classif.dummy")
rr1 = resample(task, learner1, resampling, "mmce")
rr1$experiment(1)$train.set
rr2 = resample(task, learner2, resampling, "mmce")
rr2$experiment(1)$train.set
```
