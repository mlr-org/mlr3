...
bmr = benchmark(tasks, learners, resamplings, measures, mlr_control(verbose = FALSE))
```

Usually you want to look at the results aggregated over resampling iterations:
```{r}

as.data.table(bmr)[, c("task_id", "learner_id", "acc", "auc")]
```
We can aggregate it further, i.e. if we are interested which learner performed best over all tasks:
```{r}
as.data.table(bmr)[, list(acc = mean(acc), auc = mean(auc)), by = "learner_id"]
```
...
```{r}
tab = as.data.table(bmr)[task_id == "spam" & learner_id == "classif.rpart"]
print(tab)
