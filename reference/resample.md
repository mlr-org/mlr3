...
[Learner](https://mlr3.mlr-org.com/reference/Learner.md) `learner` on a
training set of [Task](https://mlr3.mlr-org.com/reference/Task.md)
`task` to train a model, then use the trained model to predict
observations of a test set. Training and test sets are defined by the
...
instantiated on the task, making the procedure stochastic even in case
of a deterministic learner.
