...
test_that("experiment_worker", {
  ...
  expect_names(names(res), permutation.of =
    c("iteration", "learner", "train_log", "train_time", "predict_log", "predict_time", "prediction_data", "performance", "score_time"))

  expect_is(res$prediction_data, "PredictionData")
  expect_is(res$prediction_data, "PredictionDataClassif")
})
