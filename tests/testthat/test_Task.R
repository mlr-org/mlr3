...
test_that("cbind/rbind works", {
  ...
  task$cbind(data)
  expect_task(task)
  expect_equal(task$nfeatures, 5L) # "foo" was added as a feature
  ...
  expect_equal(task$nfeatures, 5L) # adding rows doesn't change #features
...
test_that("Task$set_col_roles", {
  task = tsk("pima")
  expect_equal(task$nfeatures, 8L)

  task$set_col_roles("mass", remove_from = "feature")
  expect_equal(task$nfeatures, 7L)
