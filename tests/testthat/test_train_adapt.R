test_that("learner train adapt works", {
  ...
  # increased train_adapt parameter
  ...
  # added non-train_adapt parameter
  learner_1 = lrn("classif.debug", iter = 1)
  learner_1$train(task)
  train_adapt_id = learner_1$model$train_adapt_id
...
test_that("resample train adapt works", {
  ...
  pmap(list(rr$learners, rr_2$learners), function(l1, l2) {
    ...
    expect_equal(l1$model$train_adapt_id, l2$model$train_adapt_id)
  })
})
...
test_that("benchmark train adapt works", {
  ...
  map(bmr_2$resample_result(1)$learners, function(l1) {
    ...
    expect_true(l1$model$train_adapt_id %in% train_adapt_ids)
  })
  ...
  bmr  = benchmark(design, store_models = TRUE, allow_train_adapt = TRUE)
  ...
  expect_equal(bmr$resample_result(1)$learners[[1]]$model$train_adapt_id, 
    hot$stack$start_learner[[1]]$model$train_adapt_id)
