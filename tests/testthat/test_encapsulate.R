...
test_that("encapsulate methods produce the same results", {
  ...
  learner = lrn("classif.debug")
  learner$train(task)
  expect_equal(learner$model$random_number, 26803)
  expect_equal(sample(seq(1000), 1), 526)
  rr = resample(task, learner, rsmp("cv", folds = 3), store_models = TRUE)
  expect_equal(map_int(rr$learners, function(learner) learner$model$random_number), c(8298, 48560, 21948))
