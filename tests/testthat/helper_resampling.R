expect_grouping_works = function(r) {
  ...
  for (i in seq_len(r$iters)) {
    ...
    expect_integer(r$validation_set(i), lower = 1L, upper = 150L, any.missing = FALSE)
    if (!inherits(r, "ResamplingInsample")) {
      expect_length(intersect(data[r$train_set(i), get("grp")], data[r$validation_set(i), get("grp")]), 0L)
    }
  }
}
