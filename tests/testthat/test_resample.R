...
test_that("resample with multiple measures", {
  ...
  tab = rr$performance(measures)
  expect_data_table(tab, ncol = length(measures), nrow = 3L)
  expect_equal(names(tab), ids(measures))

  perf = rr$aggregated(measures)
...
test_that("predict_type is checked", {
  ...
  expect_error(rr$aggregated(measure), "predict_type")
})
