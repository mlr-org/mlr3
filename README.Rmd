...
# train/test split
train_set = sample(task_mtcars$nrow, 0.8 * task_mtcars$nrow)
test_set = setdiff(seq_len(task_mtcars$nrow), train_set)

# train the model
learner$train(task_mtcars, row_ids = train_set)

# predict data
prediction = learner$predict(task_mtcars, row_ids = test_set)
...
resampling = mlr_resamplings$get("cv", param_vals = list(folds = 10L))
rr = resample(task_mtcars, learner, resampling)
